...
void encapsulation_demo() {
    std::cout << "----- 面向对象核心：封装（Encapsulation） -----" << std::endl;
    ...
    public:
        ...
        void setRadius(double r) {
            ...
            else std::cout << "错误：半径必须大于0！" << std::endl;
        }
        // 公有行为成员：计算面积（接口）
    ...
    std::cout << "圆形半径=5.0，面积=" << c1.getArea() << std::endl;
    ...
    class Person {
    private:
        std::string name;    // 私有数据：姓名
        int age = 0;    // 私有数据：年龄（默认初始化0）
        std::string password;// 私有数据：密码（敏感信息）
    public:
        // 1. 读写接口：姓名（可修改可读取）
        ...
        void setAge(int a) {
            ...
            else std::cout << "错误：年龄必须在1-150之间！" << std::endl;
        }
    ...
    // 操作姓名（读写）
    p.setName("Alice");
    std::cout << "Person姓名：" << p.getName() << std::endl;

    // 操作年龄（带验证的只读）
    p.setAge(25); // 合法年龄
    std::cout << "Person年龄：" << p.getAge() << std::endl;
    p.setAge(200); // 非法年龄，触发提示
    std::cout << "非法年龄后，Person年龄：" << p.getAge() << std::endl;
...
void constructor_delegation_demo() {
    std::cout << "----- 构造函数委托（Constructor Delegation） -----" << std::endl;
    ...
    public:
        ...
        Employee(const std::string& n, int a) : name(n), age(a) {
            std::cout << "目标构造函数调用：姓名=" << name << "，年龄=" << age << std::endl;
        }

        // 2. 委托构造函数：仅传姓名，年龄默认0，委托给目标构造函数
        Employee(const std::string& n) : Employee(n, 0) { // 核心："类名(参数)"委托
            std::cout << "委托构造函数调用：年龄默认0" << std::endl;
        }
    };

    // 使用构造函数
    std::cout << "创建emp1（传姓名+年龄）：" << std::endl;
    Employee emp1("John", 30); // 调用目标构造函数

    std::cout << "\n创建emp2（仅传姓名）：" << std::endl;
...
void ENfriend_function_demo() {
    ...
    std::cout << "rect1（宽3，高4）面积：" << getArea(rect1) << std::endl;
    std::cout << "rect2（宽5，高6）面积：" << getArea(rect2) << std::endl;
